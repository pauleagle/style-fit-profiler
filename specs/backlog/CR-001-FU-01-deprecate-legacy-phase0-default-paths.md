...
- The first remaining actionable slice is `CR-001-FU-01B` README manual command
  docs cleanup.
- `CR-001-FU-01C` should follow to clarify root `SPEC.md` without rewriting the
  deterministic v0.1.0 Phase 0 baseline.
- `CR-001-FU-01D` should handle older EXP / notebook / raw backlog wording after
  README and root `SPEC.md` are stable.
...
| `CR-001-FU-01B` | next | README manual command docs cleanup | Update README examples so default single/batch commands produce CR-001 native artifacts, and legacy-only `--raw` / `--prompt-file` examples include `--backend legacy`. |
| `CR-001-FU-01C` | pending | Root Phase 0 contract wording | Separate deterministic v0.1.0 `style_gene_candidates.json` baseline from CR-001 native manual Gemini defaults. Do not rewrite accepted deterministic Phase 0 behavior. |
| `CR-001-FU-01D` | pending | Legacy prompt/schema and planning-doc wording | Review EXP-001 / EXP-002 / EXP-003, notebook docs, and raw backlog future examples. Label compatibility behavior without promising non-existent flags. |
...
Next recommended step: checkpoint this DA-reviewed spec, then start
`CR-001-FU-01B` README manual command docs cleanup. Do not start projection work
or add new legacy CLI aliases as part of this follow-up unless a later accepted
spec explicitly changes the projection policy.
